		...
		<div id="list-producto" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="precioVenta" title="${message(code: 'producto.precioVenta.label', default: 'Precio Venta')}" />
					
						<g:sortableColumn property="cantidadExistencia" title="${message(code: 'producto.cantidadExistencia.label', default: 'Cantidad Existencia')}" />
				...
				<g:each in="${productoInstanceList}" status="i" var="productoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: productoInstance, field: "precioVenta")}</td>
					
						<td>${fieldValue(bean: productoInstance, field: "cantidadExistencia")}</td>

					    <td><g:link controller="carro" action="agregarCarrito" id="${productoInstance.id}">Agregar a Carro</g:link></td>
					</tr>
				</g:each>
				</tbody>
			</table>
